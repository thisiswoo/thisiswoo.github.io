...
- `일반 배열`과의 차이점은 `ArrayList`는 제한 없이 객체를 추가할 수 있다는 것이다.
...
## 2023-02-21
![2023-02-21](/assets/img/daily/routine/2023-02-26/2023-02-20_myroutine.png){:.centered width="100%"}
...
- **차이점**은 `Vector`는 동기화된(`synchronized`) 메서드로 구성되어 있기 때문에 **멀 스레드가 동시에 `Vector()` 메서드를 실행할 수 없다는 것이다**.
...
- `ArrayList`는 내부 배열에 객체를 저장하지만, `LinkedList`는 인접 객체를 체인처럼 연결해서 관리한다.
