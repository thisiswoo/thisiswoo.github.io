...
- 따라서, 이러한 전기신호를 통과시킬 수 있는 전선은 없다.
...
- 디지털 신호 그래프의 `0`은 끊어진 것처럼 보이지만 사실 끊어진 게 아니라 특정 전압 이하이기 때문에 `0`으로 그래프상 표시한 것뿐이다. 실제론 계속 전류는 흐른다.
...
- `1초`의 시간을 촘촘히 측정할 수 있게 된다면 
- 즉, 조금 더 촘촘히 많은 데이터를 수집할 수 록 `아날로그` 데이터에 굉장히 가까워진다.
- 아무리 `아날로그 신호`를 `디지털 신호`로 바꾸어도 어쩔 수 없이 데이터 왜곡은 생기기 마련이다.
...
- 위에서 예시로 든 `50ms(1초에 20번)`와 1초에 `100MHz(1초에 1,000,000번 - 100만 번)`의 데이터 수집 양에 엄청난 차이를 느낄 수 있다.
