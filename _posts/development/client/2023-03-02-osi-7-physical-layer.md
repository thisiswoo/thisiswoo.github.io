...
- 주파수가 높을수록 빛과 같이 직진성이 강해서 특정 방향으로 송신하는데 유리하고 많은 정보를 싫을 수 있다는 장점이 있다. 
- 하지만, 비가 오거나 안개가 많이 낀 날은 공기 중에 물방울과 수증기가 많아지기 때문에 전파가 흡수되어 멀리 전파될 수 없다는 단점이 있다.
- 반대로 주파수가 낮은 전파는 직진성은 약하지만 장애물을 뛰어넘는 특성이 있어 넓은 지역을 커버하는데 유리다. 하지만 주파수가 낮을수록 싫을 수 있는 데이터 정보량은 적다.
- 위에서 예시로 든 `50ms(1초에 2번)`와 1초에 `100MHz(1초에 1,000,000번 - 100만 번)`의 데이터 수집 양에 엄청난 차이를 느낄 수 있다.
